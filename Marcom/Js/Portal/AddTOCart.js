...
jQuery(document).ready(function () {
    ...
    jQuery('#CheckOut').bind('click', function () {
        ...
        if (count == 0) {
            ...
            jQuery("#messagewrapper .messagebox").text('Please Add items.');
            displayMessages();
        }
        else {
            jQuery('#Cart').hide();
...
function CheckOutUserItems() {
    ...
    if (arrayId.length > 0) {
        jQuery.ajax({
            url: '/clsWebService/AddtocardCheckout',
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            data: JSON.stringify({ arrayId: arrayId, arrayQuamtity: arrayQuamtity, CommentText: CommentText }),
            success: function (result) {
                if (result == "Checkout done succefully ,Please Check your mail") {
                    jQuery("#wrapertd").html('');
                    jQuery("#messagewrapper").html('<div class="messagebox success"></div>');
                    jQuery("#messagewrapper .messagebox").text(result);
                    displayMessages();
                }
                else {
        ...
        jQuery("#messagewrapper .messagebox").text("Please Add products to your cart at first.");
        displayMessages();
        CalculateAmount();
    }
}