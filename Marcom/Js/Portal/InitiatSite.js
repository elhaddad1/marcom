...
jQuery(document).ready(function () {
    ...
    jQuery('body').append('<div id="Cart" class="Cart_div"> <table class="Cart_Wraper" border="0" cellpadding="0" cellspacing="0">' +
            '<tr><td class="Cart_Wraper_td1"> <div class="cart_header">You have <span id="Itemcount" class="Itemcount">&nbsp 0</span> &nbsp items.</div></td></tr>' +
           ' <tr><td class="Cart_Wraper_td2"><div id="wrapertd" class="items_container"></div></td></tr>' +
            '<tr><td class="Cart_Wraper_td3"><div class="cart_Close"><a id="Close_Cart">Close</a></div> <div class="cart_Checkout"> <a id="CheckOut">Check Out</a> </div></td> </tr>' +
       '</table> </div>');
    ...
    jQuery('body').append('<div id="Cartcomment" class="Comment_div"> <table class="Comment_Wraper" border="0" cellpadding="0" cellspacing="0">' +
                        ...
                        '<textarea  id="CommentText"class="CommentTextarea" name="comment" placeholder="       Leave your message here..."></textarea>' +
                        '</div> </td> </tr> <tr><td class="Comment_Wraper_td2"> <div class="Comment_Close"><a id="BacktoCart">Back</a></div>' +
                        '<div class="Confirm"><a id="ConfirmCart">Confirm</a></div></td></tr>' +
                       ' </table> </div>');
    jQuery('#Cart').hide();
    jQuery('#Cartcomment').hide();
    jQuery.get("/menu/Brand", function (result) {
        // TODO: process the results of the server side call
        jQuery('.MUBrand').append("<a href=\"javascript:void(0);\" class=\"navlink\">Brands</a>");
        jQuery('.MUjGlide').append("<a href=\"javascript:void(0);\" class=\"navlink\">Categories</a>");
        jQuery('.MUDepts').append("<a href=\"javascript:void(0);\" class=\"navlink\">Product</a>");
        PublicBarnd = result;
        jQuery('#Brands-jGlide').hide();

        jQuery.get("/menu/Product", function (result) {
            // TODO: process the results of the server side call
            PublicDept = result;

            jQuery('#Depts-jGlide').hide();
            jQuery.get("/menu/Category", function (result) {
                // TODO: process the results of the server side call
    ...
    // don't navigate away from the field on tab when selecting an item
			...
			.autocomplete({
			    ...
			    select: function (event, ui) {
			        location.href = '/SearchAutoCmplt/Index?Str=' + ui.item.value + '';
			    }
			});
});
