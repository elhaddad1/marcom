...
jQuery.jGlideMenu = {
    ...
    loadTile: function (u, p) {
        ...
        if (jQuery.jGlideMenu.tileCount > 1) {
            // Insert Back Button
            jQuery('#' + ptr.id).append('<div class="jGM_back"><a href="#">رجوع</a></div>');
            ...
            if (jQuery.jGlideMenu.tileCount > 2) {
                // Insert Reset Button
                jQuery('#' + ptr.id).append('<div class="jGM_reset"><a href="#">الرئيسية</a></div>');
    ...
    scrollItems: function (d) {
        ...
        // Enforce Bounds
        if (c <= 0 && d == 0) return;
        if (c + jQuery.jGlideMenu.itemsToDisplay - 2 >= jQuery(s).size() && d == 1) return;
        // Handle Scroll
        if (d == 0)
    ...
    drawPagers: function (p, c) {
        jQuery('#' + p + ' .jGM_pager').find('a').each(function () {
            if (jQuery(this).attr('rel') == 'Up') {
                ...
                if (jQuery.jGlideMenu.tileScrollPosition[jQuery.jGlideMenu.tileCount] + jQuery.jGlideMenu.itemsToDisplay - 2 <
    ...
    scrollToTile: function (n, s) {
        ...
        jQuery('div#jGM_wrapper_' + jQuery.jGlideMenu.currentElementID).animate({ 'right': a }, s, jQuery.jGlideMenu.easeFx, function () {
            // Remove Extra Tiles
            if (n < t)
    ...
    buildTile: function (t, d, l) {
        ...
        // Header Layout
        template = '<div class="jGM_cats"><img src=' + t + '><p class="jGM_desc">' + d + '</p></div>';
        // Scroll Up
        template += '<div class="jGM_pager"><a href="#" rel="Up" title="Scroll Up" style="display:none"><span>المزيد</span></a></div>';
        // Items
        ...
        template += '</div>';
        // Scroll Down
        template += '<div class="jGM_pager"><a href="#" rel="Down" title="Scroll Down" style="display:none"><span>المزيد</span></a></div>';
        return template;
    },
